...
def start_timer():
    global start_time, end_time, running, is_reset, total_time, paused_time
    if running:
        pass
    else:
        if is_reset:
            ...
            paused_time = dt.datetime.now() - end_time
...
def stop_timer():
    global start_time, end_time, running, is_reset, total_time, resume_time
    if not running:
        pass
    else:
...
def reset_timer():
    global start_time, end_time, running, is_reset, total_time, resume_time
    stopwatch_label.config(text="0:00:00:00")
    total_time, start_time, end_time = empty_dt, empty_dt, empty_dt
    is_reset = True
...
rules = Label(frame, text="Try to solve the Digitle!\n\n"
...
target_widget = Label(frame, text="Target: " + str(ans) + "\n", font="Helvetica 20 bold", bg='white')
